                ...
                for j in range(len(inp)):
                    ...
                    if i < j:  # upper triangular
                        row.append(str(float(dists[(inp[a].id, inp[b].id)][0] / dists[(inp[a].id, inp[b].id)][1]))
                                   if dists[(inp[a].id, inp[b].id)][1] > 0 else str(np.nan))
...
def aggregate_mutation_rates(input_folder, output_file, verbose=False):
    ...
    s_ids = sorted(set([i for k in aggregated.keys() for i in k]))
    ...
    for i in s_ids:
        ...
        for j in s_ids:
            a, b = (i, j) if (i, j) in aggregated else (j, i) if (j, i) in aggregated else (None, None)

            if i != j and not a and not b:  # if not diagonal and both None --> missing pair
                row.append(str(np.nan))
            elif i < j:  # upper triangular
                row.append(str(float(aggregated[(a, b)][0] / aggregated[(a, b)][1]))
                           if aggregated[(a, b)][1] > 0 else str(np.nan))
            elif i > j:  # lower triangular
