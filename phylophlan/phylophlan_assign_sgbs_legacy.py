...
HOW_MANY = "10"
DOWNLOAD_URL = "http://cmprod1.cibio.unitn.it/databases/PhyloPhlAn/phylophlan_SGB_databases_v30.txt"
...
def database_list(databases_folder, update=False, exit=False, exit_value=0):
    ...
    releases_list = []
    releases_set = set()
    with open(sgbs_url) as f:
        for r in f:
            if not r.startswith('#') and (len(r.split('\t')) == 2) and ('tutorial' not in r):
                release = r.strip().split('\t')[-1].replace('.md5', '').replace('.tar', '').replace('.txt.bz2', '')
                if release not in releases_set:
                    releases_list.append(release)
                    releases_set.add(release)
